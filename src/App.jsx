...
function EpicCard({ epic, isFirst, isLast, onUp, onDown, onDragStart, onDragOverCard, onDropOnCard, dropIndicator, onOpen }) {
  ...
  return (
    <div draggable onDragStart={(e) => onDragStart(e, epic.key)} onDragOver={(e) => onDragOverCard(e, epic.key)} onDrop={(e) => onDropOnCard(e, epic.key)} style={{ position: "relative" }}>
      {dropIndicator === "before" && <div style={{ position: "absolute", top: -5, left: 0, right: 0, height: 2, borderRadius: 2, background: prod.primary }} />}
      <div onClick={() => onOpen(epic)} className="pp-card" style={{ background: T.bg1, border: `1px solid ${T.border2}`, borderRadius: 12, padding: 10, cursor: "grab", boxShadow: T.cardShadow }}>
      ...
      <div style={{ borderBottom: `1px solid ${T.border1}`, padding: "16px 24px" }}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p style={{ fontSize: 12, color: T.ink1, marginTop: 2 }}>Épicos em uma fila única — arraste entre status, use as setas pra priorizar dentro da coluna</p>
          </div>
...
function TaskCard({ task, isFirst, isLast, onUp, onDown, onDragStart, onDragOverCard, onDropOnCard, dropIndicator, onOpen }) {
  const { T, PRODUCT_STYLE, TIPO_STYLE } = useTheme();
  const { epics } = useData();
  ...
  return (
    <div draggable onDragStart={(e) => onDragStart(e, task.key)} onDragOver={(e) => onDragOverCard(e, task.key)} onDrop={(e) => onDropOnCard(e, task.key)} style={{ position: "relative" }}>
      {dropIndicator === "before" && <div style={{ position: "absolute", top: -5, left: 0, right: 0, height: 2, borderRadius: 2, background: prod.primary }} />}
      <div onClick={() => onOpen(task)} className="pp-card" style={{ background: T.bg1, border: `1px solid ${T.border2}`, borderRadius: 12, padding: 10, cursor: "grab", boxShadow: T.cardShadow }}>
        <div className="flex items-start justify-between" style={{ gap: 8 }}>
...
function StageColumn({ stage, tasks, onDropOnColumn, onDragStart, onDragOverCard, onDropOnCard, dropInfo, onOpen, moveWithinColumn }) {
  const { T } = useTheme();
...
function TaskDrawer({ task, onClose, onMove }) {
  const { T, PRODUCT_STYLE, TIPO_STYLE } = useTheme();
  if (!task) return null;
  const prod = PRODUCT_STYLE[task.project] || PRODUCT_STYLE["Backoffice"];
  return (
...
function TarefasScreen() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
      ...
      <TaskDrawer task={openTaskFull} onClose={() => setOpenTask(null)} onMove={moveTask} />
    </div>
  );
}
...
function SemanalCard({ task, onOpen, onDragStart, checked, onToggle }) {
  const { T, PRODUCT_STYLE, TIPO_STYLE } = useTheme();
  const { epics } = useData();
  const prod = PRODUCT_STYLE[task.project] || PRODUCT_STYLE["Backoffice"];
  const layer = TIPO_STYLE[layerOf(task)];
...
function SemanalDropGrid({ tasks, emptyLabel, onOpen, onDragStart, onDrop, accent, checkedKeys, onToggle }) {
  const { T } = useTheme();
  const [isOver, setIsOver] = useState(false);
  return (
    <div
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px 40px" }}>
          ...
          return (
            <div key={col.key} style={{ flex: 1, minWidth: 0 }}>
              ...
              <SemanalDropGrid
                ...
                checkedKeys={destaqueKeys} onToggle={onToggle}
              />
            </div>
          );
        })}
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop: 10 }}>
            ...
            <SemanalDropGrid tasks={otherTasks} emptyLabel="" onOpen={setOpenTask} onDragStart={onDragStart} onDrop={addToDestaques} accent="#75797d" checkedKeys={destaqueKeys} onToggle={onToggle} />
          </div>
        );
      })()}
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop: 10 }}>
            <p style={{ fontSize: 11.5, color: T.ink2, marginBottom: 6, fontFamily: "'Inter Tight', sans-serif" }}>Outras etapas ({otherTasks.length})</p>
            <SemanalDropGrid tasks={otherTasks} emptyLabel="" onOpen={setOpenTask} onDragStart={onDragStart} onDrop={removeFromDestaques} accent="#75797d" checkedKeys={destaqueKeys} onToggle={onToggle} />
          </div>
        );
      })()}

      <TaskDrawer task={openTask} onClose={() => setOpenTask(null)} onMove={() => {}} />
    </div>
  );
}
...
function EpicBar({ epic, onDragStart, onOpen, onResize, onRemove }) {
  const { T, PRODUCT_STYLE } = useTheme();
  ...
  return (
    <div
      draggable onDragStart={(e) => onDragStart(e, epic.key)} onClick={() => onOpen(epic)}
      style={{
        display: "flex", alignItems: "center", gap: 6, height: 26, borderRadius: 7, padding: "0 8px", cursor: "grab",
              ...
              <React.Fragment key={lane.product}>
                ...
                {lane.scheduled.map((e) => (
                  <div key={e.key} style={{ gridColumn: `${colOf(e.startWeek)} / span ${e.durationWeeks}`, gridRow: lane.startRow + lane.rowOf[e.key], display: "flex", alignItems: "center", zIndex: 1 }}>
                    <EpicBar epic={e} onDragStart={onDragStart} onOpen={() => setOpenKey(e.key)} onResize={resizeEpic} onRemove={removeFromGantt} />
                  </div>
                ))}
              </React.Fragment>
            );
